...
type serverConn interface {
	writeString(framer Framer, formatter Formatter, p Priority, hostname, app, tag, s string) error
	close() error
}
...
// priority and prefix.
func New(priority Priority, app, tag string) (w *Writer, err error) {
	return Dial("", "", priority, app, tag)
}
...
func Dial(network, raddr string, priority Priority, app, tag string) (*Writer, error) {
	return DialWithTLSConfig(network, raddr, priority, app, tag, nil)
}
...
func DialWithCustomDialer(network, raddr string, priority Priority, app, tag string, customDial DialFunc) (*Writer, error) {
	if customDial == nil {
		return nil, ErrNilDialFunc
	}
	return dialAllParameters(network, raddr, priority, app, tag, nil, customDial)
}
...
func DialWithTLSCertPath(network, raddr string, priority Priority, app, tag, certPath string) (*Writer, error) {
	serverCert, err := ioutil.ReadFile(certPath)
	if err != nil {
		return nil, err
	}

	return DialWithTLSCert(network, raddr, priority, app, tag, serverCert)
}
...
func DialWithTLSCert(network, raddr string, priority Priority, app, tag string, serverCert []byte) (*Writer, error) {
	...
	return DialWithTLSConfig(network, raddr, priority, app, tag, &config)
}
...
func DialWithTLSConfig(network, raddr string, priority Priority, app, tag string, tlsConfig *tls.Config) (*Writer, error) {
	return dialAllParameters(network, raddr, priority, app, tag, tlsConfig, nil)
}

// implementation of the various functions above
func dialAllParameters(network, raddr string, priority Priority, app, tag string, tlsConfig *tls.Config, customDial DialFunc) (*Writer, error) {
