...
func (n *localConn) writeString(framer Framer, formatter Formatter, p Priority, hostname, app, tag, msg string) error {
	if framer == nil {
	...
	_, err := n.conn.Write([]byte(framer(formatter(p, hostname, app, tag, msg))))
	return err
}
